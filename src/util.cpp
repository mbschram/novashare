...
#else
    const char* pszModule = "novashare";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "novashare.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "novashared.pid"));
